...
def possible_move(soduku, allowed_number,i,j):
    row_numbers = soduku[i] # we index into this list first when indexing into the array so it will take the whole row
    if allowed_number in row_numbers: 
        return False
    column_numbers = [soduku[i][j] for i in range (9)]# the double indexing here chooses the exact values and the for loop makes sure we go to the next iteration of the row
...
while solving_puzzle:
    try:
        ...
        print('You will have to start again! Please be careful and make sure you are inputimg numbers only!')
        continue        
